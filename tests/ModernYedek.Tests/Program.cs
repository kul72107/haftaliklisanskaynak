...
static async Task TestUpdateManifest()
{
        ...
        [manifestUrl] = """
            {
              "version": "1.0.7",
              "mandatory": true,
              "url": "https://updates.test/releases/ModernYedek-1.0.7.zip",
    ...
    Assert(result.Manifest.Version == "1.0.7", "update version");
}

static async Task TestUpdateDownloadAvoidsLockedStaleZip()
{
    var root = CreateTempRoot();
    var url = "https://updates.test/releases/ModernYedek-1.0.7.zip";
    var payload = "fake update payload";
    var sha256 = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(payload)));
    var staleZipPath = Path.Combine(root, "ModernYedek-1.0.7.zip");
    ...
    Assert(Path.GetFileName(path).StartsWith("ModernYedek-1.0.7-", StringComparison.OrdinalIgnoreCase), "download path has version prefix");
